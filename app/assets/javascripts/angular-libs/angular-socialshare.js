        ...
        configure: function configure(configuration) {
          ...
          if (configuration && configuration.length > 0) {
            for (; configIndex < configuration.length; configIndex += 1) {
              if (
                configuration[configIndex].provider &&
                socialshareProviderNames.indexOf(configuration[configIndex].provider) > -1
              ) {
      ...
      function socialshareDirective($window, socialshareConf, $log) {
        var linkingFunction = function linkingFunction($scope, element, attrs) {
          ...
          attrs.socialshareUrl =
            attrs.socialshareUrl || configurationElement.conf.url || configurationElement.conf.href;
