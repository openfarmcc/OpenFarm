openFarmApp.controller("gardenCtrl", [
  "$scope",
  "$http",
  "$rootScope",
  "userService",
  ...
  function gardenCtrl($scope, $http, $rootScope, userService, gardenService, cropService) {
    ...
    $scope.$watch("profileUser", function () {
      if ($rootScope.profileUser) {
        gardenService.getGardensForUser($rootScope.profileUser, function (success, response) {
          if (success) {
            ...
            $scope.profileUser.gardens.forEach(function (garden) {
              ...
              garden.garden_crops.forEach(function (garden_crop) {
                ...
                if (garden_crop.guide) {
                  cropService.getCrop(garden_crop.guide.crop_id, callback);
                }
              });
            });
          }
    ...
    $scope.destroyGarden = function (index, garden) {
      ...
      var answer = confirm(
        "Permanently delete garden " + garden.name + "? All crops stored in your garden will be " + "destroyed as well."
      );
    ...
    $scope.saveGardenCropChanges = function (garden) {
      angular.forEach(garden.garden_crops, function (crop) {
        gardenService.saveGardenCrop(garden, crop);
      });
    };
    ...
    $scope.deleteSelected = function (garden) {
      garden.garden_crops.forEach(function (crop) {
        if (crop.selected && !crop.hide) {
          ...
          gardenService.deleteGardenCrop(garden, crop, removeFromList);
        }
      });
    };
