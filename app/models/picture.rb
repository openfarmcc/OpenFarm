...
class Picture
  ...
  has_mongoid_attached_file :attachment,
                            ...
                            convert_options: {
                              all: '-background transparent -flatten +matte', small: '-gravity center -extent 100x100'
                            }
    ...
    def from_url(file_location, parent)
      ...
      if Paperclip::Attachment.default_options[:storage].to_s != 'filesystem'
        ...
        unless file_location.include?('/system/') || file_location.include?('missing.png')
          file_location = "#{Rails.root.join('public')}/#{file_location}" unless file_location.include?('http')
          pic.attachment = open(file_location)
        end
      end
      pic.save!
