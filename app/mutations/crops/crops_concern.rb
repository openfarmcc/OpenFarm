  ...
  module CropsConcern
    ...
    def validate_images
      images && images.each do |pic|
        ...
        if pic[:id]
          exist_pic = @crop.pictures.find(pic[:id])
    ...
    def maybe_delete(images)
      ...
      current_ids = @crop.pictures.pluck(:id).map(&:to_s)
      ...
      @crop.pictures.find(do_delete).map(&:destroy)
    end

    def maybe_create(images)
      images.map do |pic|
        Picture.from_url(pic[:image_url], @crop) if !pic[:id].present?
      end
    end
  end
end
