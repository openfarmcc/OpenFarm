...
describe Api::V1::GuidesController, type: :controller do
  ...
  describe 'create' do
    it 'create guides' do
      ...
      data = { attributes: { name: 'brocolini in the desert', overview: 'something exotic' }, crop_id: crop.id.to_s }
    ...
    it 'creates a crop if no crop was found' do
      ...
      data = {
        attributes: { name: 'brocolini in the desert', overview: 'something exotic' },
        crop_id: nil,
        crop_name: 'Test Crop'
      }
    ...
    it 'create guide should return an error when wrong info is passed' do
      sign_in FactoryBot.create(:user)
      data = { attributes: { overview: 'A tiny pixel test image.' }, crop_id: FactoryBot.create(:crop).id.to_s }
    ...
    it 'uploads a featured_image' do
      params = {
        attributes: { name: 'Just 1 pixel.', overview: 'A tiny pixel test image.' },
        images: [{ image_url: 'http://placehold.it/1x1.jpg' }],
        crop_id: FactoryBot.create(:crop).id.to_s
      }
      sign_in FactoryBot.create(:user)
      VCR.use_cassette('controllers/api/api_guides_controller_spec') { post 'create', params: { data: params } }
      expect(response.status).to eq(201)
  ...
  describe 'destroy' do
    ...
    it 'returns an error when a guide does not exist' do
      ...
      expect(json['errors'][0]['title']).to include('Could not find a guide with id')
      expect(response.status).to eq(422)
    end
    ...
    it 'only destroys guides owned by the user' do
      ...
      expect(json['errors'][0]['title']).to include('can only destroy guides that belong to you.')
      expect(response.status).to eq(401)
    end
  end
end
