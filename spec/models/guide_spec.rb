...
describe Guide do
  ...
  it 'creates a basic_needs array if a user has a garden' do
    ...
    FactoryBot.build(:garden, user: user, soil_type: 'Loam', type: 'Outside', average_sun: 'Partial Sun')
    guide = Guide.new(user: user)
    Stage.new(guide: guide, environment: %w[Outside], soil: %w[Clay], light: ['Partial Sun'])
  ...
  it 'returns 0 percent if there are no basic_needs for a guide' do
    ...
    FactoryBot.build(:garden, user: user, soil_type: 'Loam', type: 'Outside', average_sun: 'Partial Sun')
    guide = Guide.new(user: user)
    Stage.new(guide: guide, environment: [], soil: [], light: [])
