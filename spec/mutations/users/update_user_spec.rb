...
describe Users::UpdateUser do
  ...
  let(:params) { { id: "#{current_user.id}", attributes: { mailing_list: false }, current_user: current_user } }
  let(:params_with_usersetting) do
    { id: "#{current_user.id}", attributes: {}, user_setting: { location: 'Manila' }, current_user: current_user }
  end
  ...
  it 'rejects invalid favorite guides' do
    params[:attributes][:favorited_guide_ids] = %w[123]
    result = mutation.run(params)
  ...
  it 'updates valid favorite_crop' do
    params[:user_setting] = { favorite_crop: "#{crop.id}" }
    result = mutation.run(params).result
    expect(result).to be_a(User)
  ...
  it 'rejects invalid favorite_crop' do
    params[:user_setting] = { favorite_crop: 'bla' }
    result = mutation.run(params)
    expect(result.success?).to be(false)
  ...
  it 'rejects users that are not themselves' do
    ...
    expect { result = mutation.run(params) }.to raise_exception(OpenfarmErrors::NotAuthorized)
  end

  it 'rejects users without a valid featured image' do
    params[:pictures] = [{ image_url: 'not/absoloute.png' }]
  ...
  it 'handles users that have an existing image when image already exists' do
    VCR.use_cassette('mutations/users/update_user_existing_image.rb') do
      current_user.user_setting.pictures = [Picture.new(attachment: open('http://i.imgur.com/2haLt4J.jpg'))]
      featured_image = [{ image_url: 'http://i.imgur.com/2haLt4J.jpg' }]
