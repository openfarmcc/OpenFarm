...
describe GardenPolicy do
  ...
  let (:public_garden) do
    FactoryBot.create :garden
  end
  let (:private_garden) do
    FactoryBot.create :garden, is_private: true
  end
  let (:admin) do
  ...
  context 'for a user' do
    it 'should only return gardens in scope that are public' do
      ...
      not_mine = FactoryBot.create :garden, is_private: true, name: 'not_mine', user: other_user
      shared = FactoryBot.create :garden, is_private: false, name: 'yes!', user: other_user
    ...
    it 'should only return public gardens unless they are current_user' do
      Garden.collection.drop
      other_user = FactoryBot.create :user
      mine = FactoryBot.create :garden, is_private: true, name: 'mine', user: current_user
      publicly_shared = FactoryBot.create :garden, is_private: false, name: 'not mine but OK (public)', user: other_user
      not_mine = FactoryBot.create :garden, is_private: true, name: 'not mine', user: other_user
      @p = GardenPolicy::Scope.new(current_user, Garden).resolve
    ...
    it 'should return all gardens in scope when user is admin' do
      Garden.collection.drop
      a = FactoryBot.create :garden, is_private: true, name: 'nono', user: current_user
      b = FactoryBot.create :garden, is_private: false, name: 'yes!', user: current_user
